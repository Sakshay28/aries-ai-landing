...
export function resolveQualificationGates(industry: IndustryProfile): Set<string> {
  ...
  const industryModule = INDUSTRY_MODULES[industry];
  if (industryModule) {
    for (const g of industryModule.qualificationGates) gates.add(g);
  }
  return gates;
}
...
export interface ScoringInput {
  ...
  // Accepts either the classic AIResponse shape or the enterprise
  // GeminiConversationAnalysisV2 snapshot the live pipeline passes. The rule
  // engine only reads intent / extractedData / confidence — extra fields are ignored.
  ...
  lead: {
    ...
    manual_override?: boolean | null; // enterprise manual-lock flag
    manual_stage?: string | null;     // enterprise manual stage value
...
export function calculateLeadScore(input: ScoringInput): ScoringResult {
  ...
  // Respect BOTH the legacy manual_status and the enterprise manual_override/manual_stage lock.
  const manualStage       = lead.manual_stage || lead.manual_status || null;
  const hasManualOverride = !!lead.manual_override || !!manualStage;
  ...
  const aiIntent     = aiResponse.intent ?? 'unknown';
  const aiConfidence = aiResponse.confidence ?? 0;
  const aiIgnored    = aiConfidence < AI_CONFIDENCE_THRESHOLD;
  const intentKey    = `ai_intent:${aiIntent}`;
  const intentContrib = AI_INTENT_CONTRIBUTIONS[aiIntent];
  ...
  // 9. Relationship & lifecycle signals (from lead metadata, not message text).
  //    These fire in the live pipeline where the caller supplies profile/history;
  //    pure text-scoring unit tests leave these unset so they never fire there.
  if (lead.is_repeat_customer && !allCounted.has('returning_customer')) {
    addSignal('returning_customer', 'Returning customer profile', 10, 'engagement');
  }
  if ((lead.past_bookings_count ?? 0) > 1 && !allCounted.has('repeated_visits')) {
    addSignal('repeated_visits', 'Repeated visits or bookings history', 10, 'engagement');
  }
  if (lead.last_activity_at && !allCounted.has('ghosted_14_days')) {
    const diffDays = (Date.now() - new Date(lead.last_activity_at).getTime()) / 86_400_000;
    if (diffDays >= 14) addSignal('ghosted_14_days', 'Ghosted over 14 days', -20, 'negative');
  }
  const tags = lead.tags ?? [];
  if ((tags.includes('cancelled') || tags.includes('cancelled_booking')) && !allCounted.has('cancelled_booking')) {
    addSignal('cancelled_booking', 'Cancelled booking history', -25, 'negative');
  }
  if (tags.includes('spam') && !allCounted.has('spam_lead')) {
    addSignal('spam_lead', 'Classified as spam', -100, 'negative');
  }
  if (tags.includes('duplicate') && !allCounted.has('duplicate_lead')) {
  ...
  const effectiveStatus: LeadStatus = hasManualOverride && manualStage
    ? (manualStage as LeadStatus)
    : validatedAutoStatus;

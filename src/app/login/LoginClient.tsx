...
// Rendered instead of the login form when middleware.ts detects a still-valid
// session for this browser. Without this, visiting /login while already
// signed in used to silently bounce straight back to /dashboard — so picking
// a different Google account here (prompt=select_account) never actually ran;
// people landed back on their existing account with no explanation why.
function AlreadySignedInBanner({
  ...
  async function handleSwitchAccount() {
    ...
    try {
      ...
      // Full reload (not client nav) so the now-cleared session cookies are
      // picked up before middleware/page re-render — avoids showing a stale
      // "already signed in" banner for a session that no longer exists.
  ...
  return (
    <div style={styles.formCard}>
      <p style={styles.eyebrow}>Already signed in</p>
      <h2 style={styles.formTitle}>You&apos;re signed in as {email}</h2>
      <p style={{ fontSize: 14, color: "#475569", marginBottom: 24, lineHeight: 1.6 }}>
        Switching your Google account in another tab doesn&apos;t change this. To use a
        different Aries AI account, log out first, then sign in again with Google.
      </p>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <Link
          href={redirectTo}
          style={{ ...styles.submitBtn, textDecoration: "none", textAlign: "center" as const, display: "block" }}
        >
        ...
        <button
          ...
          disabled={loggingOut}
          style={{ ...styles.googleBtn, opacity: loggingOut ? 0.7 : 1, cursor: loggingOut ? "wait" : "pointer" }}
        >
          {loggingOut ? "Logging out..." : "Log out & use a different account"}
        </button>
      </div>
    </div>
  );
...
function LoginInner({ alreadySignedInEmail }: { alreadySignedInEmail?: string }) {
    ...
    <div style={styles.root}>
      ...
      <main style={styles.right}>
        ...
        {alreadySignedInEmail ? (
          <AlreadySignedInBanner email={alreadySignedInEmail} redirectTo={redirectTo} />
        ) : (
        <div style={styles.formCard}>
...
export default function LoginPage({ alreadySignedInEmail }: { alreadySignedInEmail?: string }) {
  return (
    <Suspense fallback={<div style={{ padding: 40 }}>Loading...</div>}>
      <LoginInner alreadySignedInEmail={alreadySignedInEmail} />
    </Suspense>
  );
}

...
export default function LazyMojoBillPage() {
  const [isEditing, setIsEditing] = useState(false);
  const [invoiceNo, setInvoiceNo] = useState("ARI/2026-08/0143");
  ...
  // Billed To Details
  const [buyerName, setBuyerName] = useState("Cafe LazyMojo");
  const [buyerAddress, setBuyerAddress] = useState(
    "H1, H2, SL Marg, Opp. Genpact Jaipur, Lal Bahadur Nagar, Malviya Nagar, Jaipur, Rajasthan 302018"
  );
      ...
      <div className="max-w-2xl mx-auto mb-6 flex items-center justify-between bg-white p-3.5 rounded-xl shadow-sm border border-slate-200 print:hidden">
        <div className="flex items-center space-x-3">
          ...
          <span className="font-semibold text-sm text-slate-900">
            Aries AI — Payment Receipt (Cafe LazyMojo)
          </span>
        </div>
        <div className="flex items-center space-x-3">
          <button

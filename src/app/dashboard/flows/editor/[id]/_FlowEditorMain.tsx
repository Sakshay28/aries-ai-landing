  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center rounded-lg overflow-hidden mr-2" style={{ border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(255,255,255,0.03)' }}>
              <button onClick={undo} disabled={!canUndo} title="Undo (⌘Z)"
                className="px-2.5 py-1.5 transition-all disabled:opacity-20 hover:bg-white/[0.06]" style={{ color: 'rgba(255,255,255,0.45)' }}>
            ...
            {!isSimulating && (
              <button onClick={() => setShowPreview(v => !v)} title="WhatsApp Preview"
                className="h-8 px-3 rounded-lg text-[12px] font-medium flex items-center gap-1.5 transition-all"
                style={{ background: showPreview ? 'rgba(34,197,94,0.1)' : 'rgba(255,255,255,0.04)', color: showPreview ? '#4ade80' : 'rgba(255,255,255,0.45)', border: `1px solid ${showPreview ? 'rgba(34,197,94,0.2)' : 'rgba(255,255,255,0.06)'}` }}>
                <Smartphone className="w-3.5 h-3.5" />
              </button>
            )}

            {!isSimulating && (
              <button onClick={() => setShowRecs(v => !v)} title="Smart Recommendations"
                className="h-8 px-3 rounded-lg text-[12px] font-medium flex items-center gap-1.5 transition-all"
                style={{ background: showRecs ? 'rgba(99,102,241,0.1)' : 'rgba(255,255,255,0.04)', color: showRecs ? '#818cf8' : 'rgba(255,255,255,0.45)', border: `1px solid ${showRecs ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.06)'}` }}>
                <Lightbulb className="w-3.5 h-3.5" />
              </button>
            )}

            {routeId !== 'new' && !isSimulating && (
              <button onClick={() => setShowFlightRecorder(v => !v)} title="Flight Recorder"
                className="h-8 px-3 rounded-lg text-[12px] font-medium flex items-center gap-1.5 transition-all"
                style={{ background: showFlightRecorder ? 'rgba(239,68,68,0.1)' : 'rgba(255,255,255,0.04)', color: showFlightRecorder ? '#f87171' : 'rgba(255,255,255,0.45)', border: `1px solid ${showFlightRecorder ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.06)'}` }}>
                <Radio className="w-3.5 h-3.5" />
              </button>
            )}

            {routeId !== 'new' && !isSimulating && (
              <button onClick={() => setShowAnalytics(v => !v)} title="Flow Analytics"
                className="h-8 px-3 rounded-lg text-[12px] font-medium flex items-center gap-1.5 transition-all"
                style={{ background: showAnalytics ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.04)', color: showAnalytics ? '#34d399' : 'rgba(255,255,255,0.45)', border: `1px solid ${showAnalytics ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.06)'}` }}>
                <BarChart2 className="w-3.5 h-3.5" />
              </button>
            )}

            {routeId !== 'new' && !isSimulating && (
              <button onClick={() => setShowVersions(true)} title="Version History"
                className="h-8 px-3 rounded-lg text-[12px] font-medium flex items-center gap-1.5 transition-all"
            ...
            <button
              ...
              style={{ background: isSimulating ? 'rgba(239,68,68,0.1)' : 'rgba(255,255,255,0.05)', color: isSimulating ? '#f87171' : 'rgba(255,255,255,0.6)', border: `1px solid ${isSimulating ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.07)'}` }}>
            ...
            <button onClick={handlePublish} disabled={isPublishing}
              ...
              style={{ background: 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)', color: '#fff', boxShadow: '0 1px 12px rgba(34,197,94,0.25)' }}>
        ...
        <div className="flex-1 flex overflow-hidden relative">
          ...
          {showRecs && !isSimulating && (
            <FlowRecommendations
              ...
              onNodeFocus={(nodeId) => { useFlowStore.getState().setSelectedNodeId(nodeId); }}
            />
          )}

          {showPreview && !isSimulating && (
            <div className="w-[280px] flex-shrink-0 z-10 flex flex-col overflow-hidden"
              style={{ background: '#0B1418', borderLeft: '1px solid rgba(255,255,255,0.05)', boxShadow: '-8px 0 24px rgba(0,0,0,0.4)' }}>
          ...
          {!isSimulating && (
            <div className="w-[360px] flex-shrink-0 z-10 overflow-hidden"
              style={{
                background: 'rgba(13,17,23,0.9)', backdropFilter: 'blur(20px)',
                borderLeft: '1px solid rgba(255,255,255,0.05)', boxShadow: '-8px 0 32px rgba(0,0,0,0.5)',
                transform: selectedNodeId ? 'translateX(0)' : 'translateX(100%)',
                transition: 'transform 0.18s cubic-bezier(0.16,1,0.3,1)',
                position: selectedNodeId ? 'relative' : 'absolute', right: 0, top: 0, bottom: 0,
              }}>
              <FlowInspector />
            </div>
          )}
      ...
      {showVersions && routeId !== 'new' && (
        <FlowVersionPanel flowId={routeId} onClose={() => setShowVersions(false)} onRestored={() => { loadFlow(routeId); }} />
      )}

      {showAnalytics && routeId !== 'new' && (
        <FlowAnalytics flowId={routeId} flowName={flowName} onClose={() => setShowAnalytics(false)}
          nodeLabels={Object.fromEntries(nodes.map(n => [n.id, String((n.data as any)?.label ?? n.type)]))} />
      )}

      {publishBurst && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, pointerEvents: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ fontSize: 72, animation: 'publishPop 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards' }}>🚀</div>
          {[...Array(12)].map((_, i) => (
            <div key={i} style={{
              ...
              transform: `rotate(${i * 30}deg) translateY(-60px)`, opacity: 0,
            }} />
          ))}
        </div>
      )}
      ...
      {showFlightRecorder && routeId !== 'new' && (
        <FlowFlightRecorder flowId={routeId} flowName={flowName} onClose={() => setShowFlightRecorder(false)}
          nodeLabels={Object.fromEntries(nodes.map(n => [n.id, String((n.data as any)?.label ?? n.type)]))} />
      )}
    </>
  );
